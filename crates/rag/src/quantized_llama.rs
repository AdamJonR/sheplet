//! Quantized Llama 3.2 inference from GGUF files.
//!
//! Follows the quantized_phi3 pattern but with separate q/k/v projections,
//! separate gate/up/down MLP, and full rotary (no partial).
...
impl Mlp {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let gate = xs.apply(&self.ffn_gate)?.silu()?;
        let up = xs.apply(&self.ffn_up)?;
        (gate * up)?.apply(&self.ffn_down)
    }
}
...
impl ModelWeights {
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let max_seq_len = md_get("llama.context_length")?.to_u32()? as usize;
        let head_dim = embedding_length / head_count;
        let rms_eps = md_get("llama.attention.layer_norm_rms_epsilon")?.to_f32()? as f64;
        ...
        let (cos, sin) = precompute_freqs_cis(rope_dim, max_seq_len, rope_freq, device)?;
        ...
        for layer_idx in 0..block_count {
            ...
            let ffn_gate = QLinear::new(&ct, reader, &format!("{prefix}.ffn_gate"), device)?;
...
mod tests {
    ...
    fn test_gguf_metadata_keys() {
        // Verify we expect the right GGUF metadata key names for Llama
        let expected_keys = [
        ...
        // Just verify the keys are distinct and properly namespaced
        for key in &expected_keys {
            assert!(key.starts_with("llama."), "key should start with 'llama.': {key}");
        }
    ...
    fn test_precompute_freqs_shape() {
        ...
        let head_dim = 64;
        let max_seq_len = 128;
        let (cos, sin) = precompute_freqs_cis(head_dim, max_seq_len, 500000.0, &device).unwrap();
        assert_eq!(cos.dims(), &[max_seq_len, head_dim / 2]);
        assert_eq!(sin.dims(), &[max_seq_len, head_dim / 2]);
    }

    #[test]
    fn test_precompute_freqs_values() {
        let device = Device::Cpu;
        let (cos, sin) = precompute_freqs_cis(64, 128, 500000.0, &device).unwrap();
        // At position 0, cos should be 1.0 and sin should be 0.0
        let cos_0: Vec<f32> = cos.get(0).unwrap().to_vec1().unwrap();
        let sin_0: Vec<f32> = sin.get(0).unwrap().to_vec1().unwrap();
        for &c in &cos_0 {
